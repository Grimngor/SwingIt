...
Shader "Sprites/Sprite Color FX/Sprite Color Dissolve Border Color Luminosity"
{
  ...
  SubShader
  {
    ...
    #pragma surface surf Unlit alpha vertex:vert addshadow nofog noambient
    ...
    float _DissolveInverseOne;    float _DissolveInverseTwo;
    ...
    struct Input
{
    ...
    void surf(Input IN, inout SurfaceOutput o)
    {
	  ...
	  float3 border = lerp(0.0, _DissolveLineColor.rgb > 0.0 ? Luminosity(pixel.rgb, _DissolveLineColor.rgb) : pixel.rgb, isClear);

	  o.Albedo = lerp(border, pixel.rgb, int(dissolve.r + _DissolveAmount - _DissolveLineWitdh)) * pixel.a;
      o.Alpha = lerp(0.0, 1.0, isClear) * pixel.a;
    }
    ENDCG
  }
